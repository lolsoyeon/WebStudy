...
<%
	...
	//-- 최초 이 페이지를 요청한 사용자나
	//    로그인을 시도했지만 정상적인 로그인 과정을 거지지 못한 사용자는
	//    userNmae을 얻지 못한 상황이지만
	//    정상적인 로그인 과정을 거친 사용자는
	//    userName에 "최동현"값을 받아서 이페이지를 방문한 상황
	//    즉, SessionTest01_ok.jsp 에서 userName(key)"최동현"(value)을
	//    담아서 이페이지로 돌려보낸 상황
	...
	// ※ 세션 기본 시간(defualt)은 1800초.
	session.setMaxInactiveInterval(5);
	//-- 세션이 유지되는 시간을 5초로 변경하여 설정한 상태.
	//   이와 같이 설정을 변경 했기 때문에
	//   5초 뒤 새로 고침하면 로그아웃 된 상태를 확인~!!
	...
	<table>
		...
		<tr>
			...
			<td>
				...
				{
				%>
				<button type="button" class="btn02">일정관리</button></a> |
				<%
				}
				else
				{
			...
			<td>
				...
				{
				%>
				<button type="button" class="btn02">친구관리</button></a> |
				<%
				}
				else
				{
	...
	<table>
		<tr>
			...
			%>
				...
				<h3>이제 일정관리와 친구관리 서비스를 이용하실수 있습니다.</h3>
				<br><br>
